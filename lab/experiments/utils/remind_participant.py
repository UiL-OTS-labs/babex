...
def remind_participant(appointment: Appointment) -> None:
    ...
    subject = "ILS *Reminder* opgave experiment: {}".format(experiment.name)
    context = {
    ...
    send_template_email([appointment.participant.email], subject, "experiments/mail/reminder", context)


def _make_cancel_link() -> str:
    return parse.urljoin(settings.FRONTEND_URI, "participant/cancel/")
